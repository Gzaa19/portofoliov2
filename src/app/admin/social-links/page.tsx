...
export default function AdminSocialLinksPage() {
    ...
    const fetchSocialLinks = async () => {
        try {
            const res = await fetch("/api/social-links?all=true");
    ...
    const openEditModal = (link: SocialLink) => {
        ...
        setFormData({
            ...
            iconBg: link.iconBg || "bg-gray-800",
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {isLoading ? (
                    <div className="col-span-full text-center text-white/50 py-8">Loading...</div>
                ) : socialLinks.length === 0 ? (
                    <div className="col-span-full text-center text-white/50 py-8">
                        Belum ada social links
                    </div>
                ) : (
                    socialLinks.map((link) => (
                        <div
                            key={link.id}
                            className={`bg-gray-800/50 backdrop-blur-md border rounded-2xl p-6 transition-all ${link.isActive ? "border-white/10" : "border-red-500/30 opacity-60"
                                }`}
                        >
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-xl ${link.iconBg || "bg-gray-800"}`}>
                                        <span className="text-xl">
                                            {iconOptions.find((i) => i.name === link.iconName)?.label?.[0] || "🔗"}
                                        </span>
                                    </div>
                                    <div>
                                        <h3 className="text-white font-semibold">{link.name}</h3>
                                        <p className="text-emerald-400 text-sm">{link.username}</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => toggleActive(link)}
                                    className={`px-2 py-1 text-xs rounded-full transition-colors ${link.isActive
                            ...
                            {link.description && (
                                <p className="text-white/50 text-sm mb-4 line-clamp-2">{link.description}</p>
                            )}
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="flex-1 px-3 py-2 text-sm text-blue-400 bg-blue-500/10 hover:bg-blue-500/20 rounded-lg transition-colors"
                                >
                                    Edit
                                </button>
                                <button
                                    onClick={() => handleDelete(link.id)}
                                    className="flex-1 px-3 py-2 text-sm text-red-400 bg-red-500/10 hover:bg-red-500/20 rounded-lg transition-colors"
                                >
                                    Delete
                                </button>
                            </div>
